...
/**
 * Represents the tiles of the game
...
public final class Tiles {
    ...
    /**
     * Creates a list of the tiles that is a subset of the full list of tiles but containing the start tiles
     * @return The list of the tiles that is a subset of the full list of tiles but containing the start tiles
     */
    public static List<Tile> createSubTiles(int limit) {
        List<Tile> startTiles = TILES.stream().filter(t -> t.kind() == Tile.Kind.START).toList();
        List<Tile> normalTiles = TILES.stream().filter(t -> t.kind() == Tile.Kind.NORMAL).toList()
                .stream().limit(limit).toList();
        List<Tile> menhirTiles = new ArrayList<>(Collections.singleton(TILES.get(88)));
        menhirTiles.addAll(TILES.stream().filter(t -> t.kind() == Tile.Kind.MENHIR).toList());

        List<Tile> subTiles = new ArrayList<>();
        subTiles.addAll(startTiles);
        subTiles.addAll(normalTiles);
        subTiles.addAll(menhirTiles);
        return Collections.unmodifiableList(subTiles);
    }

    /**
     * Creates the full list of tiles
